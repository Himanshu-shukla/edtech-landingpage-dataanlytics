...
const FoundryHero = () => {
  ...
  return (
    <section className="relative min-h-[70vh] bg-gradient-to-b from-slate-50 via-white to-slate-50 overflow-hidden">
      ...
      {/* --- OPTIMIZATION START --- 
          We wrap all heavy background animations in a div.
          When isModalOpen is true, we set opacity to 0. 
          This stops the browser from trying to render complex animations behind the modal.
      */}
      <div 
        className={`absolute inset-0 transition-opacity duration-200 ${isModalOpen ? 'opacity-0' : 'opacity-100'}`}
        aria-hidden={isModalOpen} // Accessibility hint
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          <motion.div
            ...
            className="absolute -top-10 -left-40 w-96 h-48 bg-gradient-to-br from-emerald-400/30 via-teal-400/20 to-transparent rounded-full blur-3xl will-change-transform"
          />
          <motion.div
            animate={{
              scale: [1, 1.3, 1],
            ...
            className="absolute top-40 -right-40 w-[500px] h-[500px] bg-gradient-to-bl from-violet-400/30 via-purple-400/20 to-transparent rounded-full blur-3xl will-change-transform"
          />
          <motion.div
            animate={{
              scale: [1, 1.1, 1],
            ...
            className="absolute bottom-20 left-1/2 w-80 h-80 bg-gradient-to-tr from-cyan-400/20 via-blue-400/20 to-transparent rounded-full blur-3xl will-change-transform"
          />
        </div>
        ...
        {/* Animated Background Layer 3: Grid */}
        <div className="absolute inset-0 [perspective:1000px]">
          <motion.div
            ...
            className="absolute inset-0 bg-[linear-gradient(to_right,#05966915_1px,transparent_1px),linear-gradient(to_bottom,#05966915_1px,transparent_1px)] bg-[size:60px_60px] [mask-image:radial-gradient(ellipse_80%_60%_at_50%_0%,#fff_70%,transparent_100%)] will-change-transform"
            style={{ transformOrigin: 'center top' }}
          />
        </div>

        {/* Animated Background Layer 4: Noise (Only render if modal closed) */}
        {!isModalOpen && (
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center pt-8 px-4 md:px-8">
          ...
          <motion.div
            animate={{
              boxShadow: [
                ...
                "0px 4px 20px rgba(220, 38, 38, 0.4)", 
                "0px 0px 0px rgba(220, 38, 38, 0)"
              ],
              borderColor: ["#FEE2E2", "#FCA5A5", "#FEE2E2"],
            }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            className="px-8 py-4 bg-red-50 border-b border-x border-red-100 text-red-800 font-bold text-base md:text-lg tracking-wide backdrop-blur-md rounded-b-2xl flex items-center gap-3 cursor-pointer shadow-sm will-change-transform"
          >
        ...
        >
          {/* Continuous Bounce Wrapper - Added will-change-transform */}
          <motion.div
            animate={{ y: [0, -8, 0] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            className="w-full max-w-3xl z-20 will-change-transform"
          >
            <motion.button
